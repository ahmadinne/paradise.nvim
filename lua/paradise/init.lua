...
function M.colorscheme()
    ...
    elseif vim.g.paradise_config.style == 'dark' then
        M.set_options('style', 'dark')
    end
...
---Toggle between paradise styles
function M.toggle()
    local index = vim.g.paradise_config.toggle_style_index + 1
    if index > #vim.g.paradise_config.toggle_style_list then index = 1 end
    M.set_options('style', vim.g.paradise_config.toggle_style_list[index])
    M.set_options('toggle_style_index', index)
    if vim.g.paradise_config.style == 'dark' then
...
local default_config = {
    -- Main options --
    style = 'dark',    -- choose between 'dark', 'darker', 'cool', 'deep', 'warm', 'warmer' and 'dark'
...
function M.setup(opts)
    ...
    if vim.g.paradise_config.toggle_style_key then
      vim.api.nvim_set_keymap('n', vim.g.paradise_config.toggle_style_key, '<cmd>lua require("paradise").toggle()<cr>', { noremap = true, silent = true })
    end
end
